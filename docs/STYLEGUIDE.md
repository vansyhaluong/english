...
Main learning goal or primary content

Current action / primary CTA
...
Must live in the canonical global theme/token file

Typography

Must be defined globally and consumed through tokens/utilities

Spacing

Must use the shared spacing scale

Radius

Must use shared radius tokens

Component primitives

Must be reused before writing custom replacements

Motion

Must use shared duration/easing tokens

Dark visual treatment

Must follow this guide rather than component-local color guesses
...
Cards or hover states

background-deep

Deep visual zones, media overlays

General component surfaces

surface

Cards, nav containers, lightweight panels

Full-page background

surface-hover

Hovered rows, ghost buttons, selectable cards
...
Default frames and dividers

Heavy emphasis

border-hover

Hovered/focused interactive frames

Static separators

primary

Primary CTA, selected learning action, meaningful progress accent

Decorative color everywhere

ring

Focus-visible and strong keyboard selection
...
Metadata, helper copy, placeholders
...
Hero heading on compact screens

text-5xl

64px

Large hero heading when space allows
...
Flat: translucent surface + subtle border

Raised: slightly stronger surface + border + minimal shadow

Floating: popover/dialog/menu with stronger blur and a restrained deep shadow
...
One useful status/progress signal at most

Optional concise supporting description

One obvious interaction target
...
Disable action where needed

1–3 s

Spinner or concise label change

3 s+ / multi-step

Named progress stage or persistent status
...
Top fixes: identify the three highest-impact changes.

Hierarchy: verify that the primary learning goal/action is obvious.

Surface balance: check that translucent foreground surfaces remain distinguishable from the dark background without becoming heavy dark blocks.

Friction: identify unnecessary clicks, controls, labels, or containers.

Alignment and rhythm: check grid, spacing, baselines, and visual grouping.

Interaction: verify hover, focus-visible, active, loading, disabled, and error states.

Keyboard and touch: verify practical interaction paths.

Responsive behavior: check wrapping, overflow, and content priority.

Content: verify concise labels and truthful state language.

Accessibility: verify contrast, focus, semantics, and reduced motion.
...
Normalize spacing and radius.

Replace heavy opaque dark surfaces with the documented translucent hierarchy where appropriate.

Normalize hover/focus/active states.

Reuse existing primitives.

Improve hierarchy before adding decoration.

Update one coherent surface at a time rather than partially restyling unrelated components.
...
Inspect the closest existing sibling component.

Reuse the nearest existing primitive.

Prefer an existing semantic token.

Prefer the established dark-blue + light-translucent surface language.

Check whether the decision improves hierarchy, readability, and learning flow.

If two reasonable options remain and the choice changes product direction, ask before inventing a new pattern.
